...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        RequestAPI requestAPI = mapper.readValue(response.getEntity().getContent(),
                RequestAPI.class);

        response = httpClient.execute(new HttpGet(requestAPI.getUrl()));

        byte[] img = response.getEntity().getContent().readAllBytes();
        String fileName = new File(requestAPI.getUrl()).getName();
